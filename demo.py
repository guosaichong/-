...
# 获得每段视频的url
def get_url_list():
    prefix_url = "https://youku.com-youku.net/20180626/14084_f3588039/1000k/hls/"
...
async def job(session, url):
    ...
    name = url.split('/')[-1]
    # 获得名字
    ts = await session.get(url,timeout=20)
...
async def main(loop, url):
    async with aiohttp.ClientSession() as session:
        # 建立会话 session
        tasks = [loop.create_task(job(session, url[_])) for _ in range(len(get_url_list()))]
        # 建立所有任务
...
def join_video(x,y):
    ...
    f=open(new_file,"wb")
    for i in range(x,y):
        filepath="%s\80ed70a101f%03d.ts"%(filedir,i)
...
if __name__ == "__main__":
    # 单独下载
    # url_list=["https://youku.com-youku.net/20180626/14084_f3588039/1000k/hls/80ed70a101f077.ts"]
    # download(url_list)
